...
local DEFAULT_CONFIG = {
   ...
   colors = require("monoflow.palette"),
   overrides = {},
   theme = 'monoflow'
}
...
local function get_configs()
   ...
   if g.colors_name == 'monoflow' then
      configs.theme = 'monoflow'
      configs.colors = require('monoflow.palette')
   end
...
local function load(theme)
   if vim.version().minor < 7 then
      vim.notify_once("monoflow.nvim: you must use neovim 0.7 or higher")
      return
   end
